...
Xj  = 0.09;                     % Hardware Factor of accelaration of faults
...
y = (0.4 * sin(x)+ 0.6+ (randn (1,96))/20)*500;
plot (x,y)
xlabel ('Time[h]')
ylabel ('User per square km')
title ('Variation of user density with time')
grid on;
...
title ('Plot of Sustainability Metric for EURA')
%legend ('EURA')
...
UTOT= zeros (1440,1);           % Utitlity total
ut  = zeros (1440,1);           % Utitlity
ST1  = zeros (1440,1);           % Total Sustainiability
PM  = 1;                        % Power Consumed by MeNB
Pu  = 0.2;                      % Power Consumed by ueNB
cost_of_power = 0.16;           % Rate of power kwh/USD
cost_of_repair = 380;           % Rate of repair kwh/USD
...
for t = 2:1440
  ...
  ut(t) = Calc_utility (Rj,Umin,Umax,Rmin,Rmax);
  UTOT(t) = UTOT (t-1) + ut(t);
  ST1(t) = (UTOT(t) - CE(t) - CR (t));
  end

%subplot(2,2,4);
plot (ST1, 'b--o');
legend ('EURA','Always On',0)
%xlabel ('Time [h]')
%ylabel ('Sustainability')
%title ('Plot of Sustainability Metric for Always On')
grid on;